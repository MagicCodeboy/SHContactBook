...
@implementation SHPeoplePickerDelegate

#pragma mark - ABPeoplePickerNavigationControllerDelegate
-(void)peoplePickerNavigationController:(ABPeoplePickerNavigationController *)peoplePicker didSelectPerson:(ABRecordRef)person{
    [peoplePicker dismissViewControllerAnimated:YES completion:^{
        [self addToExistingContactsWithPhoneNum:self.phoneNum person:person controller:self.controller];
    }];
}
...
-(void)addToExistingContactsWithPhoneNum:(NSString *)phoneNum
    ...
    for (CFIndex i = 0; i < ABMultiValueGetCount(phoneMulti); i++) {
        ...
        NSString * label = CFBridgingRelease(ABMultiValueCopyValueAtIndex(phoneMulti, i));
    ...
    for (int i = 0; i < phones.count; i++) {
        ABMultiValueAddValueAndLabel(multiValue, (__bridge CFTypeRef)(phones[i]), (__bridge CFStringRef)(lables[i]), NULL);
    }
